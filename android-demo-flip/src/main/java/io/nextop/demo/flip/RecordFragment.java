...
//        });
    ...
    void startPreview() {
        if (!previewStarted) {
            ...
            flip.lockCamera();
            if (flip.cameraConnected && null != surfaceTexture) {
                try {
                    flip.camera.setPreviewTexture(surfaceTexture);
                    setCameraDisplayOrientation(getActivity(), flip.cameraId, flip.camera);
                    flip.camera.startPreview();
                    previewStarted = true;
                } catch (IOException e) {
                    //
                }
